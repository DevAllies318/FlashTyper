...
function main() {
  ...
  words = "air bag cat dog egg fan gun hat".split(" ");
  ...
  function writeExtraLetters(refLetters, currentWord, typed, ind) {
    let extraLetters = 0;
    currentWord.childNodes.forEach((child) => {
      if (child.classList.contains("extraLetter")) {
        extraLetters++;
      }
    });
    if (extraLetters > 16) {
  ...
  handleKeyDown = function (event) {
    ...
    if (typed === " ") {
      let previousLetter = refLetters[ind - 2];
      if (previousLetter.classList.contains("extraLetter")) {
        correctLetters--;
        incorrectLetters++;
      }
      if (ind != 0 && refLetters[ind - 1].innerHTML == " ") {
        // correctLetters -= 1; //get previous word
        previousWord = refLetters[ind - 2].parentElement;
        previousWord.childNodes.forEach((child) => {
          if (
            child.classList.contains("wrongLetter") ||
  ...
  function getResult() {
    ...
    calculations[4].innerHTML = totalWords - correctWords;
